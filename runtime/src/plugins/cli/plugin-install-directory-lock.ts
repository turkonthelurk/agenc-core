...
// - Young dead guard, or an empty, partial, or non-file lock entry younger
//   than 60s. try-lock reports the directory busy, so recovery skips it, and
//   a blocking acquire polls until the bound passes.
...
// unlinks that file, and only then drops the nonce. A symlink is removed only
// under the reclaim guard, after two looks still show that same symlink.
// Empty, partial, and other non-owner entries are removed only under that
// guard, after two looks still show the same dev/ino and bytes and the mtime
// is at least 60s old. acquiredAtMs is not a lock timeout.
...
async function classifyExisting(lockPath: string): Promise<ExistingLockAction> {
  ...
  if (info.isSymbolicLink()) {
    const outcome = await reclaimForeignEntry(lockPath, info, undefined, true);
    return outcome === "removed" ? "retry" : "wait";
  }
  if (!info.isFile()) {
    const outcome = await reclaimForeignEntry(lockPath, info, undefined, false);
    return outcome === "removed" ? "retry" : "wait";
  }
  const text = await readLockText(lockPath);
  const parsed = parseOwner(text);
  if (text === undefined || parsed === undefined) {
    const outcome = await reclaimForeignEntry(lockPath, info, text, false);
    return outcome === "removed" ? "retry" : "wait";
  }
...
async function reclaimForeignEntry(
  lockPath: string,
  seen: LockStat,
  seenText: string | undefined,
  symlink: boolean,
): Promise<ReclaimOutcome> {
  if (!symlink && Date.now() - epochMs(seen.mtimeMs) < reclaimGuardStaleMs) return "busy";
  return withReclaimGuard(lockPath, seenText ?? "", () =>
    removeForeignEntryIfUnchanged(lockPath, seen, seenText, symlink));
}
...
async function removeForeignEntryIfUnchanged(
  lockPath: string,
  seen: LockStat,
  seenText: string | undefined,
  symlink: boolean,
): Promise<ReclaimOutcome> {
  let current: LockStat | undefined;
  for (let check = 0; check < 2; check += 1) {
    current = await lstatIfPresent(lockPath);
    if (current === undefined) return "removed";
    if (current.dev !== seen.dev || current.ino !== seen.ino) return "changed";
    if (symlink) {
      if (!current.isSymbolicLink()) return "changed";
      continue;
    }
    if (Date.now() - epochMs(current.mtimeMs) < reclaimGuardStaleMs) return "changed";
    if (await readLockText(lockPath) !== seenText) return "changed";
  }
  if (current === undefined) return "removed";
  return removeReclaimTarget(lockPath, current);
}

async function removeReclaimTarget(lockPath: string, current: LockStat): Promise<ReclaimOutcome> {
  try {
    if (current.isDirectory()) await rmdir(lockPath);
    else await unlink(lockPath);
  } catch (error) {
    const code = codeOf(error);
    if (code === "ENOENT") return "removed";
    if (code === "ENOTEMPTY" || code === "EPERM" || code === "EISDIR" || code === "ENOTDIR") return "busy";
    throw error;
  }
  return "removed";
}
